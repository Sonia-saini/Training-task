...
CourseAllotment.post("/allot/:course_id", (req: any, res: any) => {
  ...
  if (
    ...
    if (min_employees > total_registered) {
      res.status(400).send({
        ...
        failure: {
          ...
          data: data.course_registration.sort(
    ...
    parse.Course_allotment = parse.course_registration
      .filter((el) => el.course_id === courseiid)
      .sort((a, b) => a.registration_id - b.registration_id);
    let x = parse.Course_allotment.map((el: any) => {
